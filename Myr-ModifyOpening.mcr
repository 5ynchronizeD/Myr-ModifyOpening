#Version 8
#BeginDescription
Last modified by: Anno Sportel (hsbcad)
2.16 - 19.03.2020

This tsl adjusts the headers above openings .

2.13 - 13.01.2020 - 
SL openings without header assigns integrated beam (DRU) as header instead

2.14 - 15.01.2020 - 
No transform on the header if it's 30x120 (It needs to be at opening for the plate)
...
/// AS - 2.00 - 29.01.2018 - Rename tsl from Myr_ModifyHeader into Myr_ModifyOpening.
/// AS - 2.01 - 30.01.2018 - Add extra beams. Update symbol.
/// AS - 2.02 - 01.02.2018 - Add extra beam to module. Set beamcode and sublabel2.
/// AS - 2.03 - 17.09.2018 - Treat integrated beams as headers if the opening is in a garage and there are no headers found.
/// AS - 2.04 - 17.09.2018 - Add extra beams above opening for garage walls.
/// AS - 2.05 - 17.09.2018 - Fix storage of created entities for garage walls.
/// AS - 2.06 - 17.09.2018 - Allow extra beams over opening if they are longer than 300mm.
/// AS - 2.07 - 17.09.2018 - Extra beams over opening can also stretch to headers.
/// AS - 2.08 - 18.09.2018 - Erase extra beam over opening if its not allowed.
/// AS - 2.09 - 18.09.2018 - Auto update sub-element information of module beams when toggeling extra beam.
/// AS - 2.10 - 18.09.2018 - Set color and material for extra beam.
/// AS - 2.11 - 18.09.2018 - Set color and material for extra beams under opening too.
/// AS - 2.12 - 23.05.2019 - Add extra beams to list of beams which was collected at an earlier stage.
/// OR - 2.13 - 13.01.2020 - SL openings without header assigns integrated beam (DRU) as header instead
...
PropString applyExtraBeamsProp(0, noYes, T("|Apply extra beams|"));
...
PropDouble extraBeamHeightProp(1, 0, T("|Height of the extra beam|"));
extraBeamHeightProp.setCategory(categories[2]);
extraBeamHeightProp.setDescription(T("|Specifies height of the extra beams|"));
...
String deleteHeaderCommand = T("Delete header");
addRecalcTrigger(_kContext, deleteHeaderCommand );

// Create an instance per opening.
...
Line lnX(csEl.ptOrg(), elX);
Line lnY(csEl.ptOrg(), elY);
...
Point3d openingCenter = .5 * (arPtOpX[0] + arPtOpX[arPtOpX.length() - 1]);
openingCenter += elY * elY.dotProduct((.5 * (arPtOpY[0] + arPtOpY[arPtOpY.length() - 1])) - openingCenter);
openingCenter += elZ * elZ.dotProduct((el.ptOrg() - elZ * .5 * el.zone(0).dH()) - openingCenter);
//Centre of opening
_Pt0 = openingCenter;
double openingWidth = elX.dotProduct(arPtOpX[arPtOpX.length() - 1] - arPtOpX[0]);
double openingHeight = elY.dotProduct(arPtOpY[arPtOpY.length() - 1] - arPtOpY[0]);
double frameThickness = el.dBeamWidth();
...
PLine openingSymbolXZ(elY);
openingSymbolXZ.createRectangle(LineSeg(openingCenter - shrinkFactor * (elX * 0.5 * openingWidth + elZ * 0.5 * frameThickness), openingCenter + shrinkFactor * (elX * 0.5 * openingWidth + elZ * 0.5 * frameThickness)), elX, elZ);
openingSymbolXZ.transformBy(elY * shrinkFactor * 0.5 * openingHeight);
dp.draw(openingSymbolXZ);
openingSymbolXZ.transformBy(-elY * shrinkFactor * openingHeight);
dp.draw(openingSymbolXZ);
PLine openingSymbolYZ(elX);
openingSymbolYZ.createRectangle(LineSeg(openingCenter - shrinkFactor * (elY * 0.5 * openingHeight + elZ * 0.5 * frameThickness), openingCenter + shrinkFactor * (elY * 0.5 * openingHeight + elZ * 0.5 * frameThickness)), elY, elZ);
openingSymbolYZ.transformBy(elX * shrinkFactor * 0.5 * openingWidth);
dp.draw(openingSymbolYZ);
openingSymbolYZ.transformBy(-elX * shrinkFactor * openingWidth);
dp.draw(openingSymbolYZ);
dp.textHeight(U(15));
dp.draw(op.openingDescr(), _Pt0, elX, elY, 0, 2, _kDevice);


int openingIsInAGarage = (el.code().left(1) == "M");
int openingWithoutHeader = (op.constrDetail().left(2) == "SL" || el.code().left(1) == "D");

//All beams
Beam allBeams[] = el.beam();
Beam allBeamsExceptHeaders[0];
//Find the headers for this opening
Beam headers[0];
Beam arBmIntegrate[0];
Beam arBmOverDoor[0];

allBeamsExceptHeaders.append(allBeams);
for( int i=0;i<allBeams.length();i++ )
{
	...
	if( sBmCode == "HB" )
	{
		...
		if( (elX.dotProduct(ptBmMin - _Pt0) * elX.dotProduct(ptBmMax - _Pt0)) < 0 )
		{
			headers.append(bm);
			allBeamsExceptHeaders = bm.filterGenBeamsNotThis(allBeamsExceptHeaders); 
		}
	}
	else if( sBmCode == "DRU" )
	{
	...
	else if(bm.module() != "" && vxBm.isParallelTo(el.vecX()))
	{
		arBmOverDoor.append(bm);
	}
}

if ((headers.length() == 0 && openingIsInAGarage) )
{
...
if( _bOnWriteEnabled && (_kExecuteKey==deleteHeaderCommand || (headers.length() == 0 && openingWithoutHeader == 0)))
{
...
Beam horizontalBeams[] = elY.filterBeamsPerpendicular(allBeamsExceptHeaders);
horizontalBeams.append(arBmOverDoor);
Beam verticalBeams[] = elX.filterBeamsPerpendicular(allBeamsExceptHeaders);
...
//Modify header
Point3d ptElementCenter = el.ptOrg() - elZ * .5 * el.zone(0).dH();
for ( int i = 0; i < headers.length(); i++) 
{
	Beam header = headers[i];
	...
	Point3d ptHeader = header.ptCen();
	int nSide = 1; //Arrow side
	if ( elZ.dotProduct(ptHeader - ptElementCenter) < 0 ) 
	{
		nSide = - 1; //Back
		
		//Addcross for header
		lstBeams.setLength(0);
		lstBeams.append(header);
		TslInst tsl;
		tsl.dbCreate(strScriptName, vecUcsX, vecUcsY, lstBeams, lstEntities, lstPoints, lstPropInt, lstPropDouble, lstPropString);
	}
	
	//Transform header
	Beam arBmTop[] = Beam().filterBeamsHalfLineIntersectSort(horizontalBeams, ptHeader, elY);
	if ( arBmTop.length() == 0 )continue;
	Beam bmTop = arBmTop[0];
	Line lnElY(ptHeader, elY);
	Point3d ptTop = lnElY.intersect( Plane(bmTop.ptCen(), bmTop.vecD(elY)), - .5 * (bmTop.dD(bmTop.vecD(elY)) + header.dD(elY)) );
	
	int headerForPlate = 0;
	
	String bmC = header.name("beamCode").token(0);
	bmC = bmC.trimLeft();
	bmC = bmC.trimRight();
	
	//Don't move header 30x120, it's for the plate
	if((header.solidWidth() == 30 && header.solidHeight() == 120) || ( header.element().code().left(1) == "D" && bmC != "HB"))
	{
		headerForPlate = 1;
	}
		
	if (!headerForPlate)
		header.transformBy(elY * elY.dotProduct(ptTop - ptHeader));
	
	
	
	//Apply beamcut
	double dDHeaderZ = header.dD(elZ);
	double dDHeaderY = header.dD(elY);
	Point3d ptBmCut = ptTop - elZ * nSide * .5 * dDHeaderZ - elY * .5 * dDHeaderY;
	BeamCut bmCutHeader(ptBmCut, elX, elY, elZ, header.solidLength(), 2 * dDHeaderY, 2 * dDHeaderZ, 0, 1, nSide);
	
	Cut cut(ptBmCut, elZ * nSide);
	Body headerBody = bmCutHeader.cuttingBody();
	
	for (int v = 0; v < verticalBeams.length(); v++) 
	{
		if(headerForPlate)
			break;
			
		Beam verticalBeam = verticalBeams[v];
		if (verticalBeam.envelopeBody().hasIntersection(headerBody)) 
		{
			...
			Point3d startBeam = verticalBeam.ptCenSolid() - elY * 0.5 * verticalBeam.solidLength();
			Point3d endBeam = verticalBeam.ptCenSolid() + elY * 0.5 * verticalBeam.solidLength();
			
			int modifySection = (elY.dotProduct(startBeam - ptBmCut) > 0 && elY.dotProduct(endBeam - (ptBmCut + elY * 2 * dDHeaderY)) < 0);
			if (modifySection) 
			{
				verticalBeam.addTool(cut);
				
				Point3d beamEdge = verticalBeam.ptCen() - elZ * nSide * 0.5 * verticalBeam.dD(elZ);
				double newSize = abs(elZ.dotProduct(ptBmCut - beamEdge));
				
				verticalBeam.setD(elZ, newSize);
				verticalBeam.transformBy(elZ * elZ.dotProduct((beamEdge + ptBmCut) / 2 - verticalBeam.ptCen()));
			}
			else 
			{
...
for( int i=0;i<arBmIntegrate.length();i++ )
{
	...
	int nSide = 1; //Arrow side
	if( elZ.dotProduct(ptIntegrate - ptElementCenter) < 0 )
	{
		nSide = -1; //Back
	}
	
	//Apply beamcut
	double dDIntegrateZ = bmIntegrate.dD(elZ);
	double dDIntegrateY = bmIntegrate.dD(elY);
	Point3d ptBmCut = ptIntegrate - elZ * nSide * .5 * dDIntegrateZ - elY * .5 * dDIntegrateY;
	BeamCut bmCut(ptBmCut, elX, elY, elZ, bmIntegrate.solidLength(), dDIntegrateY + U(6), 2 * dDIntegrateZ, 0, 1, nSide);
	
	int nNrOfBeamsChanged = bmCut.addMeToGenBeamsIntersect(verticalBeams);
}

//region Extra Beams
// Create extra beams if needed.
Map mapCreatedEntities = _Map.getMap("CreatedEntity[]");
for (int m=0;m<mapCreatedEntities.length();m++)
{
	Entity createdEntity = mapCreatedEntities.getEntity(m);
	createdEntity.dbErase();
}
_Map.removeAt("CreatedEntity[]", true);
...
addRecalcTrigger(_kContext, toggleExtraBeamsCommand);
String doubleClickAction = "TslDoubleClick";
if (_kExecuteKey == toggleExtraBeamsCommand || _kExecuteKey == doubleClickAction) 
{
	applyExtraBeams ? applyExtraBeamsProp.set(noYes[0]) : applyExtraBeamsProp.set(noYes[1]);
	applyExtraBeams = ! applyExtraBeams;
}
...
if (applyExtraBeams && (OpeningType.makeUpper() != "DOORS" && bOutside) )
{
	...
	Beam horizontalsBottom[] = Beam().filterBeamsHalfLineIntersectSort(horizontalBeams, _Pt0, -elY);
	if (horizontalsBottom.length() > 0)
	{
		Beam sill = horizontalsBottom[0];// The openings bottom beam
		if (sill.module() != "") 
		{
			Vector3d sillX = sill.vecX();
			if (elX.dotProduct(sillX) < 0)
			{
				sillX *= -1;
			}
			...
			for (int i = 0; i < beamOrigins.length(); i++)
			{
	...
	int extraBeamsAtTheTopCreated = false;
	
	Beam horizontalsTop[] = Beam().filterBeamsHalfLineIntersectSort(horizontalBeams, _Pt0, elY);
	if (horizontalsTop.length() > 0)
	{
		Beam transom = horizontalsTop[0]; // The openings top beam
		if (transom.module() != "") 
		{
			
			Vector3d transomX = transom.vecX();
			if (elX.dotProduct(transomX) < 0)
			{
				transomX *= -1;
			}
			Point3d leftOrigin = transom.ptCenSolid() - transom.vecX() * 0.5 * (transom.solidLength() - widthExtraBeam);
			leftOrigin += elY * 0.5 * transom.dD(elY);
			leftOrigin -= elZ * 0.5 * (transom.dD(elZ) - heightExtraBeam);
			Point3d rightOrigin = transom.ptCenSolid() + transom.vecX() * 0.5 * (transom.solidLength() - widthExtraBeam);
			rightOrigin += elY * 0.5 * transom.dD(elY);
			rightOrigin -= elZ * 0.5 * (transom.dD(elZ) - heightExtraBeam);
			...
			Beam horizontalBeamsIncludingHeaders[0];
			horizontalBeamsIncludingHeaders.append(horizontalBeams);
			if(headers.length() > 0) 
				horizontalBeamsIncludingHeaders.append(headers);
...
//				extraBeam.dbCreate(beamOrigin, elY, elX, elZ, U(100), widthExtraBeam, heightExtraBeam, - 1, 0, 0);
				...
				extraBeam.setSubLabel2(transom.subLabel2());
				extraBeam.setMaterial(materialExtraBeam);
				extraBeams.append(extraBeam);
				
				Beam beamsToStretchTo[] = extraBeam.filterBeamsHalfLineIntersectSort(horizontalBeamsIncludingHeaders, beamOrigin + elY * U(110), elY);
				if (beamsToStretchTo.length() > 0)
				{
					extraBeam.stretchStaticTo(beamsToStretchTo[0], _kStretchOnToolChange);
				}
				...
				if ((openingIsInAGarage || extraBeam.solidLength() > U(300) || el.code().left(1) == "D") && !bOutside)
				{
					...
					extraBeamsAtTheTopCreated = true;
				}
				else
				{
					extraBeam.dbErase();
	...
	_Map.setMap("CreatedEntity[]", mapCreatedEntities);
}
allBeams.append(extraBeams);
...
for (int b=0;b<allBeams.length();b++)
{
	...
	{
		String moduleInformationValues[0];
		String moduleInformation = bm.subLabel2();
		
		
		for (int i=0;i<20;i++)
		{
			String value = moduleInformation.token(i);
			moduleInformationValues.append(value);
		}
			
		moduleInformationValues[12] = applyExtraBeams ? "Trälister" : "";
		moduleInformation = "";
		for (int i=0;i<moduleInformationValues.length();i++)
		{
			moduleInformation+=moduleInformationValues[i];
			if ( i < (moduleInformationValues.length() - 1) )
			{
				moduleInformation += ";";
			}
		}
		bm.setSubLabel2(moduleInformation);
	}
}
